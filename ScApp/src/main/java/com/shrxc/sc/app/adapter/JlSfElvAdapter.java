...
public class JlSfElvAdapter extends BaseExpandableListAdapter {
    ...
    public JlSfElvAdapter(Context context, List<JlGroupEntity> gameList) {
    ...
    public View getChildView(final int gPos, final int cPos, boolean b, View view, ViewGroup viewGroup) {
        ...
        for (Integer key : childs.get(gPos).get(cPos).keySet()) {
            if (key == 0) {
                ...
                JlAdapterUtil.initViewState(context, childs.get(gPos).get(cPos).get(key),
                        (LinearLayout) cvh.conLayout.getChildAt(key - 1), key - 1, 0);
            }
        }
        //重置选中的内容
        ...
        cvh.zdPlTextView.setText("胜" + gameList.get(gPos).getList().get(cPos).getZspl());
        cvh.kdNameTextView.setText(gameList.get(gPos).getList().get(cPos).getGuestteam());
        cvh.kdPlTextView.setText("负" + gameList.get(gPos).getList().get(cPos).getZfpl());
        ...
        cvh.fxLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                JlAdapterUtil.changeViewSet(context, JlSfElvAdapter.this, gPos, cPos, 0, childs, null);
            }
        });
        /**
         * 胜
        ...
        cvh.conLayout.getChildAt(0).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                JlAdapterUtil.changeViewSet(context, JlSfElvAdapter.this, gPos, cPos, 1, childs, gameList);
            }
        });
        /***
         * 负
        ...
        cvh.conLayout.getChildAt(1).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                JlAdapterUtil.changeViewSet(context, JlSfElvAdapter.this, gPos, cPos, 2, childs, gameList);
            }
        });
    ...
    private class ChildViewHolder {
        ...
        private TextView xqTextView, typeTextView, timeTextView, zdNameTextView, zdPlTextView, ssfxTextView,
                kdNameTextView, kdPlTextView;

        public ChildViewHolder(View view) {

            fxImageView = view.findViewById(R.id.jl_sf_elv_fx_icon);
            fxLayout = view.findViewById(R.id.jl_sf_elv_fx_layout);
            fxDetailLayout = view.findViewById(R.id.jl_sf_elv_fx_detail_layout);
            conLayout = view.findViewById(R.id.jl_sf_elv_content_layout);
            xqTextView = view.findViewById(R.id.jl_sf_elv_xq_text);
            typeTextView = view.findViewById(R.id.jl_sf_elv_type_text);
            timeTextView = view.findViewById(R.id.jl_sf_elv_time_text);
            zdNameTextView = view.findViewById(R.id.jl_sf_elv_zd_name_text);
            zdPlTextView = view.findViewById(R.id.jl_sf_elv_zd_pl_text);
            kdNameTextView = view.findViewById(R.id.jl_sf_elv_kd_name_text);
            kdPlTextView = view.findViewById(R.id.jl_sf_elv_kd_pl_text);
            ssfxTextView = view.findViewById(R.id.jl_sf_child_ssfx_text);
        }
    }
}