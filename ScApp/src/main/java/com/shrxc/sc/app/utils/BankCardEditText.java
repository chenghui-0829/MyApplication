...
public class BankCardEditText extends EditText {
    ...
    private TextWatcher watcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start, int before,
                                  int count) {
            System.out.println("s:" + s + ",start:" + start + ",before:"
                    + before + ",count:" + count);
            if (s == null) {
                return;
            }
                ...
                if (!isMiddle || count > 1) {
                    ...
                    if (count == 0) {
                        ...
                        if ((start - before + 1) % 5 == 0) {
                            setSelection((start - before) > 0 ? start - before
                                    : 0);
                        } else {
                            setSelection((start - before + 1) > sb.length() ? sb
                                    .length() : (start - before + 1));
                        }
                    }
                    // 如果是增加
                    else {
                        if ((start - before + count) % 5 == 0) {
                            setSelection((start + count - before + 1) < sb
                                    .length() ? (start + count - before + 1)
                                    : sb.length());
                        } else {
